...
CREATE TABLE Planecrash (			
            ...
            cn_ln				varchar(1000),
            Passenger_aboard	varchar(1000),
			Crew_aboard			varchar(1000),
            Passenger_fatalities varchar(1000),
			Crew_fatalities		varchar(1000),
            ground				varchar(1000),
...
update Planecrash
set Accident_Time = '16:50:00'
where Accident_Time = 'c16:50'

update planecrash
set accident_time = concat('0',trim(Accident_Time))
where len(trim(accident_time)) = 7

update planecrash
set Accident_Date = dateadd(year,-100,Accident_Date)
where datepart(year,accident_date) > 2018

update planecrash
set Accident_Time = '10:00:00'
where accident_time = ' 10;00:00'

update planecrash
set Accident_Time = '13:00:00'
where accident_time = ' 13;00:00'


update planecrash
set Accident_Time = '12:30:00'
where accident_time =' 12;30:00'

update planecrash
set Accident_Time = '18:04:00'
where Accident_Time = '18:.4:00'

update Planecrash
set Accident_Time = '13:15:00'
...
select count(*) as 'Accident Count',datepart(MONTH,accident_date) as Month from Planecrash group by datepart(MONTH,accident_date) order by count(*) desc
select count(*) as 'Accident Count',datepart(day,accident_date) as Day from Planecrash group by datepart(day,accident_date) order by count(*) desc
select count(*) as 'Accident Count',datepart(HOUR,Accident_Time) as Time from Planecrash group by datepart(HOUR,Accident_Time) order by count(*) desc

select count(*) as 'Accident Count',Location as Time from Planecrash group by Location order by count(*) desc
select count(*) as 'Accident Count',Operator as Time from Planecrash group by Operator order by count(*) desc
select count(*) as 'Accident Count',route as Time from Planecrash group by route order by count(*) desc
